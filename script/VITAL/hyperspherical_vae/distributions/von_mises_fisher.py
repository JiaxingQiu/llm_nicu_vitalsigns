...
class VonMisesFisher(torch.distributions.Distribution):
    ...
    def mean(self):
        ...
        ) # numerical issue with Bessel functions at high order and small input.
        # option 2:
    ...
    def entropy(self):
        ...
        log_kappa = torch.log(kappa + 1e-8)
        log_iv = v * torch.log(kappa / 2 + 1e-8) - 0.5 * torch.log(2 * math.pi * kappa + 1e-8)

        log_c = (
            -v * log_kappa
            - (self.__m / 2.0) * math.log(2 * math.pi)
            - kappa
            + log_iv
        )

        # Approximate entropy = -κ * A(κ) + log C_d(κ)
        entropy = -kappa * (v / (kappa + 1e-8)) + log_c

        return entropy.view(*(kappa.shape))
